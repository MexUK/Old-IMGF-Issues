...
{
	SetWindowText(getWindowHwnd(), CStringUtility::convertStdStringToStdWString(getWindowTitle()).c_str());
}
	...
	for (auto pWTDEntry : vecWTDEntries)
	{
		...
		else
		{
			...
			*/
			...
			pTextureData->m_strTextureFormat = CImageUtility::getD3DFormatText(pWTDEntry->getD3DFormat());
		}
...
void				CTextureViewer::openWindow(void)
{
	...
	if (!RegisterClassEx(&wc))
	{
		...
		CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_52", uiError), CLocalizationManager::getInstance()->getTranslatedText("TextPopup_Title52"), MB_OK);
		return;
	}
	...
	{
		CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_53"), CLocalizationManager::getInstance()->getTranslatedText("TextPopup_Title52"), MB_OK);
		return;
	}
...
void			CTextureViewer::renderDisplayType_Single(void)
{
	...
	for (auto pImageData : getKGM()->getEntryViewerManager()->getTextureViewer()->getEntries())
	{
		...
		if (false)
		{
			...
			SIZE textSize;
			GetTextExtentPoint32(hdc, CStringUtility::convertStdStringToStdWString(pImageData->m_strDiffuseName).c_str(), pImageData->m_strDiffuseName.length(), &textSize);
			uiCalculatedWidth = textSize.cx;

			SIZE textSize2;
			GetTextExtentPoint32(hdc, CStringUtility::convertStdStringToStdWString(pImageData->m_strAlphaName).c_str(), pImageData->m_strAlphaName.length(), &textSize2);
			...
			//CDebugger::log("IMAGE X = " + CStringUtility::toString(uiImageX));
			...
			rect.right = 8000;
			rect.bottom = 8000;
			DrawText(hdc, CStringUtility::convertStdStringToStdWString(pImageData->m_strDiffuseName).c_str(), pImageData->m_strDiffuseName.length(), &rect, DT_NOPREFIX);
			...
			rect.right = 8000;
			rect.bottom = 8000;
			DrawText(hdc, CStringUtility::convertStdStringToStdWString(pImageData->m_strAlphaName).c_str(), pImageData->m_strAlphaName.length(), &rect, DT_NOPREFIX);
		...
		else
		{
			...
			rect.right = 8000;
			rect.bottom = 8000;
			string strText = CStringUtility::toString(uiTextureIndex + 1);
			DrawText(hdc, CStringUtility::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX);
			...
			rect.right = 8000;
			rect.bottom = 8000;
			strText = CStringUtility::toString(pImageData->m_uiWidth) + " x " + CStringUtility::toString(pImageData->m_uiHeight);
			DrawText(hdc, CStringUtility::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX); // size
			...
			DrawText(hdc, CStringUtility::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX); // BPP
			...
			DrawText(hdc, CStringUtility::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX); // raster format
	...
	rect.right = 8000;
	rect.bottom = 8000;
	DrawText(hdc, CStringUtility::convertStdStringToStdWString(pActiveImageData->m_strDiffuseName).c_str(), pActiveImageData->m_strDiffuseName.length(), &rect, DT_NOPREFIX); // diffuse name

	rect.left = g_uiLeftPanelWidth + 5;
	rect.top = 25;
	rect.right = 8000;
	rect.bottom = 8000;
	DrawText(hdc, CStringUtility::convertStdStringToStdWString(pActiveImageData->m_strAlphaName).c_str(), pActiveImageData->m_strAlphaName.length(), &rect, DT_NOPREFIX); // alpha name

	rect.left = g_uiLeftPanelWidth + 120 + 5;
	rect.top = 6;
	rect.right = 8000;
	rect.bottom = 8000;
	string strText = CStringUtility::toString(pActiveImageData->m_uiWidth) + " x " + CStringUtility::toString(pActiveImageData->m_uiHeight);
	DrawText(hdc, CStringUtility::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX); // size
	...
	rect.right = 8000;
	rect.bottom = 8000;
	strText = "0x" + CStringUtility::toStringHex(pActiveImageData->m_uiRasterFormat);
	DrawText(hdc, CStringUtility::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX); // raster format
	*/
