...
void						RenderText(string text, GLfloat x, GLfloat y, GLfloat scale, vector<uint8> color)
{
	...
	//if (glGetError()) CGUIUtility::showMessage("glUseProgram error", "Error");

	GLint iLocation = glGetUniformLocation(ProgramObject, "textColor");
	if (glGetError()) CGUIUtility::showMessage("glGetUniformLocation error", "Error");

	//glUniform3f(glGetUniformLocation(ProgramObject, "textColor"), color[0], color[1], color[2]);
	glUniform3f(iLocation, 0.9, 0.1, 0.1);
	if (glGetError()) CGUIUtility::showMessage("glUniform3f error", "Error");

	glActiveTexture(GL_TEXTURE0);
	if (glGetError()) CGUIUtility::showMessage("glActiveTexture error", "Error");

	glBindVertexArray(VAO);
	if (glGetError()) CGUIUtility::showMessage("glBindVertexArray error", "Error");
...
void						handleCursorMove(GLFWwindow* window, double xpos, double ypos)
{
	...
	CVector2D vecNewPosition = CVector2D(xpos, ypos);
	if (g_vecLastMousePosition.m_x != 0.0f && g_vecLastMousePosition.m_y != 0.0f)
	{
		...
		vecCameraRotation.m_z += CMathUtility::convertDegreesToRadians(vecPositionDifference.m_x * vecCursorMoveMultiplier.m_x);
		vecCameraRotation.m_x += CMathUtility::convertDegreesToRadians(vecPositionDifference.m_y * vecCursorMoveMultiplier.m_y);
	}
...
void						CCollisionViewer::initGLEW(void)
{
	...
	if (err != GLEW_OK)
	{
		CGUIUtility::showMessage("Failed to load GLEW library.", "Library Load Failed", MB_OK);
	}
}
...
void						CCollisionViewer::moveCamera(uint8 ucAxisIndex, bool bPositivePositionChange, float32 fMoveDistance)
{
	...
	case 0:
	{
		...
		vecCameraPosition = CMathUtility::getPositionInFrontOfPosition(vecCameraPosition, fAngleRad, fMoveDistance);
		//vecCameraLookAtPosition = CMathUtility::getPositionInFrontOfPosition(vecCameraLookAtPosition, fAngleRad, fPositionChange);
		break;
	}
	case 1:
	{
		float32 fAngleRad = CMathUtility::convertDegreesToRadians(90.0f) + vecCameraRotation.m_z;
		vecCameraPosition = CMathUtility::getPositionInFrontOfPosition(vecCameraPosition, fAngleRad, fMoveDistance);
		//vecCameraLookAtPosition = CMathUtility::getPositionInFrontOfPosition(vecCameraLookAtPosition, fAngleRad, fPositionChange);
		break;
	}
	...
	//vecCameraRotation.m_z = CMathUtility::getAngleBetweenPoints(vecCameraPosition, vecCameraLookAtPosition) + CMathUtility::convertDegreesToRadians(90.0f);
}
void						CCollisionViewer::zoomCamera(float32 fMoveDistance)
{
	float32 fXAngle = vecCameraRotation.m_x + CMathUtility::convertDegreesToRadians(90.0f);
	float32 fZAngle = vecCameraRotation.m_z + CMathUtility::convertDegreesToRadians(90.0f);
	CVector3D vecCameraPositionOffset = CMathUtility::getCartesianFromSpherical(fMoveDistance, fXAngle, fZAngle);
...
void						CCollisionViewer::init3DSceneShader(void)
{
	...
	if (glGetError()) CGUIUtility::showMessage("glCreateShader v", "Error");
	fragmentShader2 = glCreateShader(GL_FRAGMENT_SHADER);
	if (glGetError()) CGUIUtility::showMessage("glCreateShader f", "Error");
	...
	uint32 uiFileSize1 = CFileUtility::getFileSize(strVertexShadersFilePath);
	...
	string strData1 = CFileUtility::getFileContent(strVertexShadersFilePath, false);
	...
	uint32 uiFileSize2 = CFileUtility::getFileSize(strFragmentShadersFilePath);
	...
	string strData2 = CFileUtility::getFileContent(strFragmentShadersFilePath, false);
	...
	if (glGetError()) CGUIUtility::showMessage("glShaderSource v", "Error");
	glShaderSource(fragmentShader2, 1, files2, lengths2);
	if (glGetError()) CGUIUtility::showMessage("glShaderSource f", "Error");
	//glShaderSourceARB
	...
	if (glGetError()) CGUIUtility::showMessage("glCompileShader v 3d", "Error");
	glCompileShader(fragmentShader2);
	if (glGetError()) CGUIUtility::showMessage("glCompileShader f", "Error");
	...
	if (!compiled)
	{
		CGUIUtility::showMessage("glGetShaderiv compile error v", "Error");
	}

	glGetShaderiv(fragmentShader2, GL_COMPILE_STATUS, &compiled);
	if (!compiled)
	{
		CGUIUtility::showMessage("glGetShaderiv compile error f", "Error");
	}

	// link shader
	ProgramObject_3DScene = glCreateProgram();
	if (glGetError()) CGUIUtility::showMessage("glCreateProgram error", "Error");

	glAttachShader(ProgramObject_3DScene, vertexShader2);
	if (glGetError()) CGUIUtility::showMessage("glAttachShader v error", "Error");
	glAttachShader(ProgramObject_3DScene, fragmentShader2);
	if (glGetError()) CGUIUtility::showMessage("glAttachShader f error", "Error");

	glLinkProgram(ProgramObject_3DScene);
	if (glGetError()) CGUIUtility::showMessage("glLinkProgram error", "Error");

	GLint linked;
	glGetProgramiv(ProgramObject_3DScene, GL_LINK_STATUS, &linked);
	if (!linked)
	{
		CGUIUtility::showMessage("glGetProgramiv link error", "Error");
	}
	...
	if (!iError)
	{
		...
		else
		{
			...
			if (!iError)
			{
				...
				CGUIUtility::showMessage("failed to load char", "Error");
				continue;
				}
				...
				if (glGetError()) CGUIUtility::showMessage("glCompileShader v text", "Error");
				...
				if (glGetError()) CGUIUtility::showMessage("glGenVertexArrays error", "Error");

				glGenBuffers(1, &VBO);
				if (glGetError()) CGUIUtility::showMessage("glGenBuffers", "Error");

				glBindVertexArray(VAO);
				if (glGetError()) CGUIUtility::showMessage("glBindVertexArray", "Error");

				glBindBuffer(GL_ARRAY_BUFFER, VBO);
				if (glGetError()) CGUIUtility::showMessage("glBindBuffer", "Error");

				glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 6 * 4, NULL, GL_DYNAMIC_DRAW);
				if (glGetError()) CGUIUtility::showMessage("glBufferData", "Error");

				glEnableVertexAttribArray(0);
				if (glGetError()) CGUIUtility::showMessage("glEnableVertexAttribArray", "Error");

				glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), 0);
				if (glGetError()) CGUIUtility::showMessage("glVertexAttribPointer error", "Error");

				glBindBuffer(GL_ARRAY_BUFFER, 0);
				if (glGetError()) CGUIUtility::showMessage("glBindBuffer error", "Error");
...
void						CCollisionViewer::prepareRenderData(void)
{
	...
	vecCameraPosition = CMathUtility::getPositionInFrontOfPosition(vecCameraPosition, 45.0f, fHighestDistance * fDistanceMultiplier);
	vecCameraRotation = { 0.0f, 0.0f, CMathUtility::getAngleBetweenPoints(vecCameraPosition, vecCameraLookAtPosition) + CMathUtility::convertDegreesToRadians(90.0f) };
...
void						CCollisionViewer::render(void)
{
	...
	*/
	...
	//if (glGetError()) CGUIUtility::showMessage("test1 error", "Error");

	//glScissor(0, 0, 100, 100);
	//glEnable(GL_SCISSOR_TEST);

	prepare2DRender();
	if (glGetError()) CGUIUtility::showMessage("test1.5 error", "Error"); 
	renderBackground();

	if (glGetError()) CGUIUtility::showMessage("test2 error", "Error");
	...
	if (glGetError()) CGUIUtility::showMessage("test3 error", "Error");

	prepare2DRender();
	if (glGetError()) CGUIUtility::showMessage("test3.5 error", "Error");
	render2DObjects();

	if (glGetError()) CGUIUtility::showMessage("test4 error", "Error");
...
void						CCollisionViewer::prepare2DRender(void)
{
	...
	if (glGetError()) CGUIUtility::showMessage("prepare2DRender glMatrixMode 1 error", "Error");

	//glPushMatrix();
	//int yaz = glGetError();
	//if (yaz) CGUIUtility::showMessage("prepare2DRender glPushMatrix error", "Error");

	glLoadIdentity();
	if (glGetError()) CGUIUtility::showMessage("prepare2DRender glLoadIdentity 1 error", "Error");
	...
	if (glGetError()) CGUIUtility::showMessage("prepare2DRender glOrtho error", "Error");

	glMatrixMode(GL_MODELVIEW);
	if (glGetError()) CGUIUtility::showMessage("prepare2DRender glMatrixMode 2 error", "Error");

	//glPushMatrix();
	glLoadIdentity();
	if (glGetError()) CGUIUtility::showMessage("prepare2DRender glLoadIdentity 2 error", "Error");

	glDisable(GL_CULL_FACE);
	if (glGetError()) CGUIUtility::showMessage("prepare2DRender glDisable(GL_CULL_FACE) error", "Error");

	//glClear(GL_DEPTH_BUFFER_BIT);
	glClear(GL_DEPTH_BUFFER_BIT);
	if (glGetError()) CGUIUtility::showMessage("prepare2DRender glClear(GL_DEPTH_BUFFER_BIT)", "Error");
}
...
void						CCollisionViewer::renderBackground(void)
{
	...
	if (glGetError()) CGUIUtility::showMessage("BG glDepthMask error", "Error");
	glDisable(GL_DEPTH_TEST);  // disable depth-testing
	if (glGetError()) CGUIUtility::showMessage("BG glDisable error", "Error");
	...
	glBegin(GL_QUADS);
	//if (glGetError()) CGUIUtility::showMessage("BG glBegin error", "Error");
	CVector2D vecBackgroundArea(w, h);
	...
	//if (glGetError()) CGUIUtility::showMessage("BG glShadeModel error", "Error");
	uint32 i = 0;
	glColor3ub(120, 120, 120);
	//if (glGetError()) CGUIUtility::showMessage("BG glColor3ub 1 error", "Error");
	for (auto vecVertex : vecBackgroundPoints)
	{
		if (i == 2)
		{
			glColor3ub(73, 73, 73);
			//if (glGetError()) CGUIUtility::showMessage("BG glColor3ub 2 error", "Error");
		}

		glVertex2f(vecVertex.m_x, vecVertex.m_y);
		//if (glGetError()) CGUIUtility::showMessage("BG glVertex2f error", "Error");

		i++;
	}
	glEnd();
	//int a = glGetError();
	//if (a) CGUIUtility::showMessage("BG glEnd error", "Error");

	glEnable(GL_DEPTH_TEST);
	if (glGetError()) CGUIUtility::showMessage("BG glEnable error", "Error");
...
{
	// camera rotation
	glRotatef(CMathUtility::convertRadiansToDegrees(vecCameraRotation.m_x), 1.0f, 0.0f, 0.0f); // Rotate our camera on the x-axis (looking up and down)
	glRotatef(CMathUtility::convertRadiansToDegrees(vecCameraRotation.m_z), 0.0f, 1.0f, 0.0f); // Rotate our camera on the y-axis (looking left and right)
...
void						CCollisionViewer::renderBoundingSphere(void)
{
	...
	{
		CVector3D vecPosition = CMathUtility::getCartesianFromSpherical(pCOLEntry->getBoundingObjects().m_fRadius, CMathUtility::convertDegreesToRadians(i), CMathUtility::convertDegreesToRadians(0.0f));
		...
		//CDebugger::log("m_fRadius: " + CStringUtility::toString(pCOLEntry->getBoundingObjects().m_fRadius) + ", vecPosition: " + CStringUtility::toString(vecPosition.m_x) + ", " + CStringUtility::toString(vecPosition.m_y) + ", " + CStringUtility::toString(vecPosition.m_z));
	}
	glEnd();
	...
	for (uint32 i = 0, j = 360, step = 1; i < j; i += step)
	{
		CVector3D vecPosition = CMathUtility::getCartesianFromSpherical(pCOLEntry->getBoundingObjects().m_fRadius, CMathUtility::convertDegreesToRadians(90.0f), CMathUtility::convertDegreesToRadians(i));
		vecPosition = vecPosition + pCOLEntry->getBoundingObjects().m_vecCenter;
		glVertex3f(vecPosition.m_x, vecPosition.m_z, vecPosition.m_y);
		//CDebugger::log("m_fRadius: " + CStringUtility::toString(pCOLEntry->getBoundingObjects().m_fRadius) + ", vecPosition: " + CStringUtility::toString(vecPosition.m_x) + ", " + CStringUtility::toString(vecPosition.m_y) + ", " + CStringUtility::toString(vecPosition.m_z));
	}
	...
	for (uint32 i = 0, j = 360, step = 1; i < j; i += step)
	{
		CVector3D vecPosition = CMathUtility::getCartesianFromSpherical(pCOLEntry->getBoundingObjects().m_fRadius, CMathUtility::convertDegreesToRadians(i), CMathUtility::convertDegreesToRadians(90.0f));
		vecPosition = vecPosition + pCOLEntry->getBoundingObjects().m_vecCenter;
		glVertex3f(vecPosition.m_x, vecPosition.m_z, vecPosition.m_y);
		//CDebugger::log("m_fRadius: " + CStringUtility::toString(pCOLEntry->getBoundingObjects().m_fRadius) + ", vecPosition: " + CStringUtility::toString(vecPosition.m_x) + ", " + CStringUtility::toString(vecPosition.m_y) + ", " + CStringUtility::toString(vecPosition.m_z));
	}
...
void						CCollisionViewer::renderBoundingCuboid(void)
{
	...
	vector<CVector3D> vecVertices = CMathUtility::getCuboidFaceVerticesAsQuads(pCOLEntry->getBoundingObjects().m_vecMin, pCOLEntry->getBoundingObjects().m_vecMax);
...
void						CCollisionViewer::renderCollisionMeshes(void)
{
	...
	for (TFace& face : pCOLEntry->getCollisionMeshFaces())
	{
		...
		//CDebugger::log("vecVector1: " + CStringUtility::toString(vecVector1.m_x) + ", " + CStringUtility::toString(vecVector1.m_y) + ", " + CStringUtility::toString(vecVector1.m_z));
		//CDebugger::log("vecVector2: " + CStringUtility::toString(vecVector2.m_x) + ", " + CStringUtility::toString(vecVector2.m_y) + ", " + CStringUtility::toString(vecVector2.m_z));
		//CDebugger::log("vecVector3: " + CStringUtility::toString(vecVector3.m_x) + ", " + CStringUtility::toString(vecVector3.m_y) + ", " + CStringUtility::toString(vecVector3.m_z));
...
void						CCollisionViewer::renderCollisionCuboids(void)
{
	...
	//CDebugger::log("collisionBox.m_min: " + CStringUtility::toString(collisionBox.m_min.m_x) + ", " + CStringUtility::toString(collisionBox.m_min.m_y) + ", " + CStringUtility::toString(collisionBox.m_min.m_z));
	//CDebugger::log("collisionBox.m_max: " + CStringUtility::toString(collisionBox.m_max.m_x) + ", " + CStringUtility::toString(collisionBox.m_max.m_y) + ", " + CStringUtility::toString(collisionBox.m_max.m_z));
	uint32 i = 0;
	int i2 = 0;
	int colors[6][3] = {
		{ 255, 0, 0 },
	...
	{
		vector<CVector3D> vecVertices = CMathUtility::getCuboidFaceVerticesAsQuads(collisionBox.m_min, collisionBox.m_max);
		i = 0;
		i2 = 0;
		for (auto vecVertex : vecVertices)
		{
			//CDebugger::log("vecVertex: " + CStringUtility::toString(vecVertex.m_x) + ", " + CStringUtility::toString(vecVertex.m_y) + ", " + CStringUtility::toString(vecVertex.m_z));
			if ((i % 4) == 0)
			{
...
void						CCollisionViewer::render2DObjects(void)
{
	...
	if (aa)
	{
		CGUIUtility::showMessage("glDepthMask error", "Error");
		int h;
		h = 0;
	}
	glDisable(GL_DEPTH_TEST);  // disable depth-testing
	if (glGetError()) CGUIUtility::showMessage("glDisable error", "Error");
	f++;

	renderLeftPanelBackground();
	if (glGetError()) CGUIUtility::showMessage("AAA error", "Error");
	renderBottomPanelBackground();
	if (glGetError()) CGUIUtility::showMessage("AAB error", "Error");
	renderPanelLineSeparators();
	if (glGetError()) CGUIUtility::showMessage("AAC error", "Error");



	//g_FontTest1
	if (glGetError()) CGUIUtility::showMessage("AA error", "Error");
	glEnable(GL_BLEND);
	if (glGetError()) CGUIUtility::showMessage("glEnable error", "Error");
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	if (glGetError()) CGUIUtility::showMessage("glBlendFunc error", "Error");
	...
	if (glGetError()) CGUIUtility::showMessage("glGetFloatv 1 error", "Error");

	GLfloat fModelViewMatrix[16];
	glGetFloatv(GL_MODELVIEW_MATRIX, fModelViewMatrix);
	if (glGetError()) CGUIUtility::showMessage("glGetFloatv 2 error", "Error");

	GLint iLocation1 = glGetUniformLocation(ProgramObject, "projection");
	if (glGetError()) CGUIUtility::showMessage("glGetUniformLocation 1 error", "Error");

	GLint iLocation2 = glGetUniformLocation(ProgramObject, "modelview");
	if (glGetError()) CGUIUtility::showMessage("glGetUniformLocation 2 error", "Error");

	glUniformMatrix3fv(iLocation1, 1, GL_FALSE, fProjectionMatrix);
	if (glGetError()) CGUIUtility::showMessage("glUniformMatrix3fv 1 error", "Error");

	glUniformMatrix3fv(iLocation2, 1, GL_FALSE, fModelViewMatrix);
	if (glGetError()) CGUIUtility::showMessage("glUniformMatrix3fv 2 error", "Error");
	*/
	...
	memset(&cdata, 'z', 95);
	memcpy((&cdata) + 95, 0, 1);
	string strFontData = CFileUtility::getFileContent("arial.ttf");
...
void						CCollisionViewer::render2DText(void)
{
	...
	*/
	...
	if (hdc == NULL)
	{
		CGUIUtility::showMessage("GetDC() failed", "Error", MB_OK);
	}
	...
	rect.right = 8000;
	rect.bottom = 8000;
	DrawText(hdc, CStringUtility::convertStdStringToStdWString("Test").c_str(), CStringUtility::convertStdStringToStdWString("Test").length(), &rect, DT_NOPREFIX);
...
void						CCollisionViewer::renderPanelLineSeparators(void)
{
	...
	glBegin(GL_QUADS);
	glColor3ub(0, 0, 0);
	CVector2D vecBackgroundArea2(w, h);
