...
void						testFormatSerialization(string strFormatFilePath)
{
	...
	// test storing
	string strOutputFormatFilePath = "Output/" + CPathUtility::getFileName(strFormatFilePath);
	...
	if(!CFileUtility::compareFiles(strFormatFilePath, strOutputFormatFilePath))
	{
		pFormat->unload();
		delete pFormat;
