...
void									CGUIManager::updateGUIControlsTextToLanguage(CDialog *pDialog, string strWindowName)
{
	...
	for (auto it : umapTranslatedTextEntries)
	{
		...
		if (CStringUtility::doesStartWith(strInternalName, strStartText, false))
		{
			string strTextRemaining = strInternalName.substr(strStartText.length());

			if (CStringUtility::toUpperCase(strTextRemaining) == "TITLE")
			{
				// e.g. Window_DFFConversion_Title
				SetWindowText(pDialog->GetSafeHwnd(), CStringUtility::convertStdStringToStdWString(strLanguageText).c_str());
			}
			else if (CStringUtility::isPositiveInteger(strTextRemaining))
			{
				// e.g. Window_DFFConversion_100
				uint32 uiControlId = CStringUtility::toNumber(strTextRemaining);
				pDialog->GetDlgItem(uiControlId)->SetWindowText(CStringUtility::convertStdStringToStdWString(strLanguageText).c_str());
				//CWnd::FromHandle(GetDlgItem(pDialog->GetSafeHwnd(), uiControlId))->SetWindowText(CStringUtility::convertStdStringToStdWString(strLanguageText).c_str());
			}
		}
	}
}
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		pDialog->SetWindowTextW(CStringUtility::convertStdStringToStdWString(p3ButtonDialogData->m_strWindowTitle).c_str());
		((CStatic*)pDialog->GetDlgItem(100))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(p3ButtonDialogData->m_strStaticText).c_str());

		((CStatic*)pDialog->GetDlgItem(150))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(p3ButtonDialogData->m_strButton1Text).c_str());
		((CStatic*)pDialog->GetDlgItem(151))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(p3ButtonDialogData->m_strButton2Text).c_str());
		((CStatic*)pDialog->GetDlgItem(152))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(p3ButtonDialogData->m_strButton3Text).c_str());
...
uint32							CGUIManager::show3ButtonDialog(string strWindowTitle, string strStaticText, string strButton1Text, string strButton2Text, string strButton3Text)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		pDialog->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextInputDialogData->m_strWindowTitle).c_str());
		((CStatic*)pDialog->GetDlgItem(1))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextInputDialogData->m_strStaticText).c_str());
		((CStatic*)pDialog->GetDlgItem(0))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextInputDialogData->m_strEditText).c_str());
	...
	case WM_COMMAND:
		...
		case 3:
			...
			((CTextInputDialogData*)GetWindowLong(hwndDlg, GWL_USERDATA))->m_strResultText = CStringUtility::convertCStringToStdString(strText);
			EndDialog(hwndDlg, 0);
			break;
		}
		break;
...
string				CGUIManager::showTextInputDialog(string strWindowTitle, string strStaticText, string strEditText)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "ConvertIMG");

		pConvertDialogData = (CConvertDialogData*)lParam;
		SetWindowLong(hwndDlg, GWL_USERDATA, (DWORD)pConvertDialogData);

		pDialog->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pConvertDialogData->m_strWindowTitle).c_str());
		((CButton*)pDialog->GetDlgItem(5))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pConvertDialogData->m_strButtonText).c_str());
		break;
	case WM_COMMAND:
		switch (wParam)
		{
...
uint32		CGUIManager::showConvertDialog(string strWindowTitle, string strButtonText)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Export");
...
uint32		CGUIManager::showExportViaDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		pTextInputDialogData = (CTextInputDialogData*)lParam;
		pDialog->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextInputDialogData->m_strWindowTitle).c_str());
		((CStatic*)pDialog->GetDlgItem(0))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextInputDialogData->m_strStaticText).c_str());
		((CStatic*)pDialog->GetDlgItem(1))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextInputDialogData->m_strEditText).c_str());
...
string				CGUIManager::showTextAreaDialog(string strWindowTitle, string strStaticText, string strEditText)
{
	CTextInputDialogData *pTextInputDialogData = new CTextInputDialogData;
	pTextInputDialogData->m_strWindowTitle = strWindowTitle;
	pTextInputDialogData->m_strStaticText = strStaticText;
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Sort");
...
vector<int>			CGUIManager::showSortOptionsDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Remove");
...
uint32		CGUIManager::showRemoveViaDialog(void)
{
	CExportViaDialogData *pExportViaDialogData = new CExportViaDialogData;
	pExportViaDialogData->m_bRadioButtonSelected = false;
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Import");
...
uint32		CGUIManager::showImportViaDialog(void)
{
	CExportViaDialogData *pExportViaDialogData = new CExportViaDialogData;
	pExportViaDialogData->m_bRadioButtonSelected = false;
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Stats");
		...
		for (auto it : pStatsDialogData->m_umapStatsExtensions)
		{
			...
			((CListCtrl*)pDialog->GetDlgItem(1))->InsertItem(uiItemIndex, CStringUtility::convertStdStringToStdWString(it.first).c_str());
			((CListCtrl*)pDialog->GetDlgItem(1))->SetItem(uiItemIndex, 1, LVIF_TEXT, CStringUtility::convertStdStringToStdWString(CStringUtility::toString(it.second)).c_str(), 0, 0, 0, 0);
		}
		...
		for (auto it : pStatsDialogData->m_umapStatsRWVersions)
		{
			...
			uiItemIndex = ((CListCtrl*)pDialog->GetDlgItem(1))->GetItemCount();
			((CListCtrl*)pDialog->GetDlgItem(1))->InsertItem(uiItemIndex, CStringUtility::convertStdStringToStdWString(strRWVersionName).c_str());
			((CListCtrl*)pDialog->GetDlgItem(1))->SetItem(uiItemIndex, 1, LVIF_TEXT, CStringUtility::convertStdStringToStdWString(CStringUtility::toString(it.second)).c_str(), 0, 0, 0, 0);
		}
	...
	case WM_COMMAND:
		...
		case 0:
			...
			break;
		case 5:
			strSaveFilePath = CGUIUtility::saveFileDialog(getKGM()->getLastUsedDirectory("SAVE_STATS"), "TXT", CLocalizationManager::getInstance()->getTranslatedText("SaveFilePopup_1_InitialFilename"));
			if (strSaveFilePath != "")
			{
				...
				for (int i = 0; i < ((CListCtrl*)pDialog->GetDlgItem(1))->GetItemCount(); i++)
				{
					...
					strSaveData += CStringUtility::convertCStringToStdString(strText1) + "\t" + CStringUtility::convertCStringToStdString(strText2) + "\n";
				}
				CFileUtility::storeFile(strSaveFilePath, strSaveData, false, false);
			}
			break;
		}
		break;
...
void				CGUIManager::showStatsDialog(
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "OrphanEntries");
		...
		((CStatic*)pDialog->GetDlgItem(101))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pOrphanEntriesData->m_strStaticText).c_str());
		((CStatic*)pDialog->GetDlgItem(102))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pOrphanEntriesData->m_strButtonText).c_str());

		for (auto strEntryName : pOrphanEntriesData->m_vecEntryNames)
		{
			uiItemIndex = ((CListCtrl*)pDialog->GetDlgItem(100))->GetItemCount();
			((CListCtrl*)pDialog->GetDlgItem(100))->InsertItem(uiItemIndex, CStringUtility::convertStdStringToStdWString(strEntryName).c_str());
		}
...
bool				CGUIManager::showOrphanEntriesDialog(vector<string>& vecEntryNames, string strStaticText, string strButtonText)
{
	...
	)
{
	...
	case WM_INITDIALOG:
	{
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Settings");
		...
		// quick export path
		((CEdit*)pDialog->GetDlgItem(104))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pSettingsData->m_strQuickExportPath).c_str());
		...
		// automatic logging
		((CEdit*)pDialog->GetDlgItem(110))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pSettingsData->m_strAutomaticLoggingPath).c_str());
	...
	case WM_COMMAND:
		...
		case 105: // save
		{
			...
			pSettingsData->m_strQuickExportPath = CStringUtility::convertCStringToStdString(strText);
			...
			pSettingsData->m_strAutomaticLoggingPath = CStringUtility::convertCStringToStdString(strText);
			...
			if (ePreviousLanguage != eNewLanguage)
			{
				...
				uint32 uiResult2 = CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_61"), CLocalizationManager::getInstance()->getTranslatedText("TextPopupTitle_61"), MB_OKCANCEL);
				if (uiResult2 == IDOK)
				{
					ShellExecute(NULL, L"open", CStringUtility::convertStdStringToStdWString(strExePath).c_str(), NULL, NULL, SW_SHOWNORMAL);
		...
		case 100: // choose (for quick export path)
			pSettingsData->m_strQuickExportPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_1"));
			((CEdit*)pDialog->GetDlgItem(104))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pSettingsData->m_strQuickExportPath).c_str());
			break;
		case 111: // choose (for automatic logging path)
			pSettingsData->m_strAutomaticLoggingPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_2"));
			((CEdit*)pDialog->GetDlgItem(110))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pSettingsData->m_strAutomaticLoggingPath).c_str());
			break;
		}
		break;
...
bool				CGUIManager::showSettingsDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		((CStatic*)pDialog->GetDlgItem(100))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pConfirmDialogData->m_strStaticText).c_str());
		pDialog->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pConfirmDialogData->m_strWindowTitle).c_str());
...
bool				CGUIManager::showConfirmDialog(string strStaticText, string strWindowTitle)
{
...
bool				CGUIManager::showConfirmDialog(string strStaticText, string strWindowTitle, bool& bDidCancel)
{
	CConfirmDialogData *pConfirmDialogData = new CConfirmDialogData;
	pConfirmDialogData->m_strStaticText = strStaticText;
	pConfirmDialogData->m_strWindowTitle = strWindowTitle;
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Dump");
		...
		((CEdit*)pDialog->GetDlgItem(113))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDumpDialogData->m_strLastUsedPath__Destination).c_str());
	...
	case WM_COMMAND:
		...
		case 112: // dump
			...
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_2"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
				break;
			}
			...
			if (pDumpDialogData->m_uiDumpType == 2) // DAT file
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_3"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
				if (pDumpDialogData->m_strGameDirectoryPath == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_4"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
			...
			if (pDumpDialogData->m_uiDumpType == 3) // Game IMGs
			{
				...
				if (pDumpDialogData->m_strGameDirectoryPath == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_5"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
		...
		case 115: // Change button for DAT file path
			vecFilePaths = CGUIUtility::openFileDialog(pDumpDialogData->m_strLastUsedPath__DAT, "DAT", false);
			if (vecFilePaths.size() == 0)
			{
				break;
			}
			pDumpDialogData->m_strDATPath = vecFilePaths[0];
			((CEdit*)pDialog->GetDlgItem(116))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDumpDialogData->m_strDATPath).c_str());
			break;
		case 144: // Change button for Game folder path
		case 148:
			pDumpDialogData->m_strGameDirectoryPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_3"), getKGM()->getLastUsedDirectory("DUMP__GAME"));
			if (pDumpDialogData->m_strGameDirectoryPath == "")
			{
				break;
			}
			pDumpDialogData->m_strGameDirectoryPath = CPathUtility::addSlashToEnd(pDumpDialogData->m_strGameDirectoryPath);
			getKGM()->setLastUsedDirectory("DUMP__GAME", pDumpDialogData->m_strGameDirectoryPath);
			((CEdit*)pDialog->GetDlgItem(143))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDumpDialogData->m_strGameDirectoryPath).c_str());
			((CEdit*)pDialog->GetDlgItem(149))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDumpDialogData->m_strGameDirectoryPath).c_str());
			break;
		case 110: // Change button for dump destination folder path
			pDumpDialogData->m_strDumpDestinationFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_4"), pDumpDialogData->m_strLastUsedPath__Destination);
			if (pDumpDialogData->m_strDumpDestinationFolderPath == "")
			{
				break;
			}
			pDumpDialogData->m_strDumpDestinationFolderPath = CPathUtility::addSlashToEnd(pDumpDialogData->m_strDumpDestinationFolderPath);
			((CEdit*)pDialog->GetDlgItem(113))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDumpDialogData->m_strDumpDestinationFolderPath).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CDumpDialogData*		CGUIManager::showDumpDialog(string strLastUsedPath__DAT, string strLastUsedPath__Destination)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "ListView");

		pDialog->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pListViewDialogData->m_strWindowTitle).c_str());

		((CListCtrl*)pDialog->GetDlgItem(100))->InsertColumn(0, CStringUtility::convertStdStringToStdWString(pListViewDialogData->m_strCol1Text).c_str(), 0, 200);
		((CListCtrl*)pDialog->GetDlgItem(100))->SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0, LVS_EX_FULLROWSELECT);

		((CStatic*)pDialog->GetDlgItem(101))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pListViewDialogData->m_strStaticText).c_str());

		for (auto strLineText : pListViewDialogData->m_vecLines)
		{
			uiItemIndex = ((CListCtrl*)pDialog->GetDlgItem(100))->GetItemCount();
			((CListCtrl*)pDialog->GetDlgItem(100))->InsertItem(uiItemIndex, CStringUtility::convertStdStringToStdWString(strLineText).c_str());
		}
	...
	case WM_COMMAND:
		...
		case 103:
			...
			break;
		case 102:
			strSaveFilePath = CGUIUtility::saveFileDialog(getKGM()->getLastUsedDirectory("LISTVIEW__" + pListViewDialogData->m_strSaveFolderPartialHandle), "TXT", pListViewDialogData->m_strSaveFileName);
			if (strSaveFilePath != "")
			{
				...
				for (int i = 0; i < ((CListCtrl*)pDialog->GetDlgItem(100))->GetItemCount(); i++)
				{
					...
					strSaveData += CStringUtility::convertCStringToStdString(strText1) + "\n";
				}
				CFileUtility::storeFile(strSaveFilePath, strSaveData, false, false);
			}
			break;
...
void				CGUIManager::showListViewDialog(string strWindowTitle, string strStaticText, string strCol1Text, vector<string>& vecLines, string strSaveFileName, string strSaveFolderPartialHandle)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		pDialog->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextureListDialogData->m_strWindowTitle).c_str());

		((CListCtrl*)pDialog->GetDlgItem(100))->InsertColumn(0, CStringUtility::convertStdStringToStdWString(pTextureListDialogData->m_strCol1Text).c_str(), 0, 200);
		((CListCtrl*)pDialog->GetDlgItem(100))->SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0, LVS_EX_FULLROWSELECT);

		((CStatic*)pDialog->GetDlgItem(101))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTextureListDialogData->m_strStaticText).c_str());

		for (auto strTextureName : pTextureListDialogData->m_vecLines)
		{
			uiItemIndex = ((CListCtrl*)pDialog->GetDlgItem(100))->GetItemCount();
			((CListCtrl*)pDialog->GetDlgItem(100))->InsertItem(uiItemIndex, CStringUtility::convertStdStringToStdWString(strTextureName).c_str());
		}
	...
	case WM_COMMAND:
		...
		case 103:
			...
			break;
		case 102:
			strSaveFilePath = CGUIUtility::saveFileDialog(getKGM()->getLastUsedDirectory("TEXTURE_LIST"), "TXT", CLocalizationManager::getInstance()->getTranslatedText("SaveFilePopup_2_InitialFilename"));
			if (strSaveFilePath != "")
			{
...
CTextureListDialogData*		CGUIManager::showTextureListDialog(string strWindowTitle, string strStaticText, string strCol1Text, vector<string>& vecLines)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "SessionManager");
		...
		for (auto strSessionData : pSessionManagerDialogData->m_vecSessionsData)
		{
			...
			{
				vecIMGNames.push_back(CPathUtility::getFileName(deqIMGPaths[i2]));
			}

			uiItemIndex = ((CListCtrl*)pDialog->GetDlgItem(100))->GetItemCount();
			((CListCtrl*)pDialog->GetDlgItem(100))->InsertItem(uiItemIndex, CStringUtility::convertStdStringToStdWString(CStringUtility::toString(uiSessionIndex + 1)).c_str());
			((CListCtrl*)pDialog->GetDlgItem(100))->SetItem(uiItemIndex, 1, LVIF_TEXT, CStringUtility::convertStdStringToStdWString(strSessionName).c_str(), 0, 0, 0, 0);
			((CListCtrl*)pDialog->GetDlgItem(100))->SetItem(uiItemIndex, 2, LVIF_TEXT, CStringUtility::convertStdStringToStdWString(CStringUtility::toString(uiIMGFileCount)).c_str(), 0, 0, 0, 0);
			((CListCtrl*)pDialog->GetDlgItem(100))->SetItem(uiItemIndex, 3, LVIF_TEXT, CStringUtility::convertStdStringToStdWString(CStringUtility::join(vecIMGNames, "; ")).c_str(), 0, 0, 0, 0);
	...
	case WM_COMMAND:
		...
		case 101:
			// add session
			strNewSessionData = getKGM()->getGUIManager()->showTextAreaDialog(CLocalizationManager::getInstance()->getTranslatedText("AddSession"), CLocalizationManager::getInstance()->getTranslatedText("Window_TextArea_1_Message"));
			if (CStringUtility::split(strNewSessionData, "\n").size() >= 2)
			{
				pSessionManagerDialogData->m_vecSessionsToAdd.push_back(CStringUtility::join(CStringUtility::split(strNewSessionData, "\n"), "; "));
				EndDialog(hwndDlg, 0);
			}
			break;
		case 102:
			...
			strSessionName = CStringUtility::convertCStringToStdString(pListControl->GetItemText(nItem, 1));

			for (auto strSessionData : pSessionManagerDialogData->m_vecSessionsData)
			{
				if (CStringUtility::split(strSessionData, "; ")[0] == strSessionName)
				{
					//CDebugger::log(strSessionData);
					strNewSessionData = getKGM()->getGUIManager()->showTextAreaDialog(CLocalizationManager::getInstance()->getTranslatedText("UpdateSession"), CLocalizationManager::getInstance()->getTranslatedText("Window_TextArea_1_Message"), CStringUtility::join(CStringUtility::split(strSessionData, "; "), "\n"));
					if (CStringUtility::split(strNewSessionData, "\n").size() >= 2)
					{
						pSessionManagerDialogData->m_strSessionNameToUpdate = strSessionName;
						pSessionManagerDialogData->m_strSessionDataToUpdate = CStringUtility::join(CStringUtility::split(strNewSessionData, "\n"), "; ");
						EndDialog(hwndDlg, 0);
					}
					break;
				}
		...
		case 103:
			...
			{
				nItem = pListControl->GetNextSelectedItem(pos);
				pSessionManagerDialogData->m_vecSessionsToRemove.push_back(CStringUtility::convertCStringToStdString(pListControl->GetItemText(nItem, 1)));
...
CSessionManagerDialogData*	CGUIManager::showSessionManagerDialog(vector<string>& vecSessionsData)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Split");
...
uint32		CGUIManager::showSplitViaDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "DragDrop");
		...
		for (auto pRWVersion : CRWManager::getInstance()->getVersionManager()->getEntries())
		{
			...
			{
				((CComboBox*)pDialog->GetDlgItem(52))->AddString(CStringUtility::convertStdStringToStdWString(pRWVersion->getVersionName() + " (" + CLocalizationManager::getInstance()->getTranslatedText(pRWVersion->getLocalizationKey()) + ")").c_str());
		...
		{
			((CComboBox*)pDialog->GetDlgItem(56))->AddString(CStringUtility::convertStdStringToStdWString(pCOLVersion->getText()).c_str());
		...
		*/
		...
		for (auto pRWVersion : CRWManager::getInstance()->getVersionManager()->getEntries())
		{
			if (pRWVersion->getVersionType() == TYPE_MODEL)
			{
				((CComboBox*)pDialog->GetDlgItem(54))->AddString(CStringUtility::convertStdStringToStdWString(pRWVersion->getVersionName() + " (" + CLocalizationManager::getInstance()->getTranslatedText(pRWVersion->getLocalizationKey()) + ")").c_str());
...
CDragDropDialogData*			CGUIManager::showDragDropDialog(CDragDropDialogData *pDragDropDialogData)
{
	pDragDropDialogData->m_bCopy = true;
	pDragDropDialogData->m_bConvert = false;
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "DumpResults");

		SetWindowText(pDialog->GetDlgItem(1)->GetSafeHwnd(), CStringUtility::convertStdStringToStdWString(CStringUtility::toString(pDumpResultsDialogData->m_uiCorruptTXDCount)).c_str());
		SetWindowText(pDialog->GetDlgItem(2)->GetSafeHwnd(), CStringUtility::convertStdStringToStdWString(CStringUtility::toString(pDumpResultsDialogData->m_uiTooLargeTXDCount)).c_str());
		SetWindowText(pDialog->GetDlgItem(3)->GetSafeHwnd(), CStringUtility::convertStdStringToStdWString(CStringUtility::toString(pDumpResultsDialogData->m_uiInvalidResolutionTXDCount)).c_str());
		SetWindowText(pDialog->GetDlgItem(4)->GetSafeHwnd(), CStringUtility::convertStdStringToStdWString(CStringUtility::toString(pDumpResultsDialogData->m_uiInvalidTextureNameCount)).c_str());
		SetWindowText(pDialog->GetDlgItem(5)->GetSafeHwnd(), CStringUtility::convertStdStringToStdWString(CStringUtility::toString(pDumpResultsDialogData->m_uiTXDCountExceededTextureCountLimit)).c_str());
		SetWindowText(pDialog->GetDlgItem(6)->GetSafeHwnd(), CStringUtility::convertStdStringToStdWString(CStringUtility::toString(pDumpResultsDialogData->m_uiMipmapSkippedCount)).c_str());
		break;
	case WM_COMMAND:
...
void				CGUIManager::showDumpResultsDialog(CDumpResultsDialogData *pDumpResultsDialogData)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Credits");
...
void				CGUIManager::showCreditsDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "Renamer");
	...
	case WM_COMMAND:
		...
		case 400: // rename button
		{
			...
			pRenamerDialogData->m_strMatchingPath_IDE = CStringUtility::convertCStringToStdString(cstrStrIDEFolder);
			pRenamerDialogData->m_strMatchingPath_IPL = CStringUtility::convertCStringToStdString(cstrStrIPLFolder);
			pRenamerDialogData->m_strMatchingPath_COL = CStringUtility::convertCStringToStdString(cstrStrCOLFolder);
			...
			if (((CButton*)pDialog->GetDlgItem(151))->GetCheck() == BST_CHECKED)
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_6"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
			if (((CButton*)pDialog->GetDlgItem(152))->GetCheck() == BST_CHECKED)
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_7"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
			if (((CButton*)pDialog->GetDlgItem(153))->GetCheck() == BST_CHECKED)
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_8"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
			...
			pRenamerDialogData->m_strCharAdding_Front = CStringUtility::convertCStringToStdString(cstrStr1);
			pRenamerDialogData->m_strCharAdding_End = CStringUtility::convertCStringToStdString(cstrStr2);
			pRenamerDialogData->m_strCharAdding_AtPosition = CStringUtility::convertCStringToStdString(cstrStr3);

			if (pRenamerDialogData->m_strCharAdding_AtPosition != "")
			{
				CString cstrStr4;
				((CEdit*)pDialog->GetDlgItem(208))->GetWindowText(cstrStr4);
				string strStr4 = CStringUtility::convertCStringToStdString(cstrStr4);

				if (strStr4 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_9"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
				pRenamerDialogData->m_uiCharAddingAtPosition_Position = CStringUtility::toNumber(strStr4);
			}
			...
			if (pRenamerDialogData->m_ucCharReplacementType == 2) // only replace if length exceeds
			{
				...
				string strStr5 = CStringUtility::convertCStringToStdString(cstrStr5);

				if (strStr5 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_10"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
				pRenamerDialogData->m_uiCharReplaceConditional_Length = CStringUtility::toNumber(strStr5);
			}
			...
			string strStr6_2 = CStringUtility::convertCStringToStdString(cstrStr6);
			string strStr7_2 = CStringUtility::convertCStringToStdString(cstrStr7);
			string strStr8 = CStringUtility::convertCStringToStdString(cstrStr8);
			if (strStr6_2 == "")
			{
			...
			{
				pRenamerDialogData->m_uiCharRemoving_Front = CStringUtility::toNumber(strStr6_2);
			}
			if (strStr7_2 == "")
			{
			...
			{
				pRenamerDialogData->m_uiCharRemoving_End = CStringUtility::toNumber(strStr7_2);
			}
			if (strStr8 == "")
			{
			...
			{
				pRenamerDialogData->m_uiCharRemoving_AtPosition = CStringUtility::toNumber(strStr8);
			}
			if (strStr8 != "")
			{
				CString cstrStr9;
				((CEdit*)pDialog->GetDlgItem(308))->GetWindowText(cstrStr9);
				string strStr9 = CStringUtility::convertCStringToStdString(cstrStr9);

				if (strStr9 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_11"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
				pRenamerDialogData->m_uiCharRemovingAtPosition_Position = CStringUtility::toNumber(strStr9);
			}
		...
		case 157: // matching IDE - folder path
			pRenamerDialogData->m_strMatchingPath_IDE = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_5"), getKGM()->getLastUsedDirectory("RENAMER__MATCHING_IDE"));
			if (pRenamerDialogData->m_strMatchingPath_IDE == "")
			{
				break;
			}
			pRenamerDialogData->m_strMatchingPath_IDE = CPathUtility::addSlashToEnd(pRenamerDialogData->m_strMatchingPath_IDE);
			getKGM()->setLastUsedDirectory("RENAMER__MATCHING_IDE", pRenamerDialogData->m_strMatchingPath_IDE);
			((CEdit*)pDialog->GetDlgItem(154))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pRenamerDialogData->m_strMatchingPath_IDE).c_str());
			break;
		case 158: // matching IPL - folder path
			pRenamerDialogData->m_strMatchingPath_IPL = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_6"), getKGM()->getLastUsedDirectory("RENAMER__MATCHING_IPL"));
			if (pRenamerDialogData->m_strMatchingPath_IPL == "")
			{
				break;
			}
			pRenamerDialogData->m_strMatchingPath_IPL = CPathUtility::addSlashToEnd(pRenamerDialogData->m_strMatchingPath_IPL);
			getKGM()->setLastUsedDirectory("RENAMER__MATCHING_IPL", pRenamerDialogData->m_strMatchingPath_IPL);
			((CEdit*)pDialog->GetDlgItem(155))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pRenamerDialogData->m_strMatchingPath_IPL).c_str());
			break;
		case 159: // matching COL - folder path
			pRenamerDialogData->m_strMatchingPath_COL = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_7"), getKGM()->getLastUsedDirectory("RENAMER__MATCHING_COL"));
			if (pRenamerDialogData->m_strMatchingPath_COL == "")
			{
				break;
			}
			pRenamerDialogData->m_strMatchingPath_COL = CPathUtility::addSlashToEnd(pRenamerDialogData->m_strMatchingPath_COL);
			getKGM()->setLastUsedDirectory("RENAMER__MATCHING_COL", pRenamerDialogData->m_strMatchingPath_COL);
			((CEdit*)pDialog->GetDlgItem(156))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pRenamerDialogData->m_strMatchingPath_COL).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CRenamerDialogData*	CGUIManager::showRenamerDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "DFFConversion");
	...
	case WM_COMMAND:
		...
		case 200: // convert button
			...
			if (pConversionDialogData->m_ucPrelightningOption == 2) // adjust prelightning
			{
				...
				strStr1 = CStringUtility::convertCStringToStdString(cstrStr1);
				strStr2 = CStringUtility::convertCStringToStdString(cstrStr2);
				strStr3 = CStringUtility::convertCStringToStdString(cstrStr3);
				strStr4 = CStringUtility::convertCStringToStdString(cstrStr4);
				if (pConversionDialogData->m_ucAdjustPrelightningOption == 0)
				{
					if (strStr1 == "" || strStr2 == "" || strStr3 == "" || strStr4 == "")
					{
						CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_12"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
						break;
					}
				}
				else if (pConversionDialogData->m_ucAdjustPrelightningOption == 1)
				{
					if (strStr1 == "" || strStr2 == "" || strStr3 == "" || strStr4 == "")
					{
						CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_13"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
						break;
					}
				}
				
				pConversionDialogData->m_ssAdjustColourValues[0] = CStringUtility::toNumber(strStr1);
				pConversionDialogData->m_ssAdjustColourValues[1] = CStringUtility::toNumber(strStr2);
				pConversionDialogData->m_ssAdjustColourValues[2] = CStringUtility::toNumber(strStr3);
				pConversionDialogData->m_ssAdjustColourValues[3] = CStringUtility::toNumber(strStr4);
			}
...
CDFFConversionDialogData*	CGUIManager::showDFFConversionDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "DuplicateEntries");
	...
	case WM_COMMAND:
		...
		case 250: // check
			...
			if (pDuplicateEntriesDialogData->m_ucEntriesType == 3) // DAT file
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_14"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
				if (pDuplicateEntriesDialogData->m_strDATGameDirectoryPath == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_15"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
		...
		case 203: // Change button for DAT file path
			vecFilePaths = CGUIUtility::openFileDialog(getKGM()->getLastUsedDirectory("DUPLICATEENTRIES__DAT"), "DAT", false);
			if (vecFilePaths.size() == 0)
			{
				break;
			}
			...
			((CEdit*)pDialog->GetDlgItem(202))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDuplicateEntriesDialogData->m_strDATPath).c_str());
			break;
		case 206: // Change button for Game folder path
			pDuplicateEntriesDialogData->m_strDATGameDirectoryPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_3"), getKGM()->getLastUsedDirectory("DUPLICATEENTRIES__GAME"));
			if (pDuplicateEntriesDialogData->m_strDATGameDirectoryPath == "")
			{
				break;
			}
			pDuplicateEntriesDialogData->m_strDATGameDirectoryPath = CPathUtility::addSlashToEnd(pDuplicateEntriesDialogData->m_strDATGameDirectoryPath);
			getKGM()->setLastUsedDirectory("DUPLICATEENTRIES__GAME", pDuplicateEntriesDialogData->m_strDATGameDirectoryPath);
			((CEdit*)pDialog->GetDlgItem(205))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDuplicateEntriesDialogData->m_strDATGameDirectoryPath).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CDuplicateEntriesDialogData*		CGUIManager::showDuplicateEntriesDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
	{
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "BuildTXD");
		...
		for (auto pRWVersion : CRWManager::getInstance()->getVersionManager()->getEntries())
		{
			((CComboBox*)pDialog->GetDlgItem(203))->InsertString(uiOptionIndex, CStringUtility::convertStdStringToStdWString(pRWVersion->getVersionName() + " (" + CLocalizationManager::getInstance()->getTranslatedText(pRWVersion->getLocalizationKey()) + ")").c_str());
			uiOptionIndex++;
		}
	...
	case WM_COMMAND:
		...
		case 150: // check
		{
			...
			pBuildTXDDialogData->m_strTexturesFolderPath = CPathUtility::addSlashToEnd(CStringUtility::convertCStringToStdString(cstr1));
			pBuildTXDDialogData->m_strDestinationFolderPath = CPathUtility::addSlashToEnd(CStringUtility::convertCStringToStdString(cstr2));
			pBuildTXDDialogData->m_strIDEFilePath = CStringUtility::convertCStringToStdString(cstr3);
			...
			if (pBuildTXDDialogData->m_uCDFFFormatsType == 3)
			{
				...
				pBuildTXDDialogData->m_strDFFsFolderPath = CPathUtility::addSlashToEnd(CStringUtility::convertCStringToStdString(cstr4));
			}
			...
			if (pBuildTXDDialogData->m_uCDFFFormatsType == 3) // Folder containg DFF files
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_16"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
			else if (pBuildTXDDialogData->m_uCDFFFormatsType == 4) // IDE file referencing DFF files
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_17"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}

				if (pBuildTXDDialogData->m_strDFFsFolderPath == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_18"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
			...
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_19"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
				break;
			}

			if (pBuildTXDDialogData->m_strDestinationFolderPath == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_20"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
				break;
			}

			CString cstr5;
			((CEdit*)pDialog->GetDlgItem(201))->GetWindowText(cstr5);
			string str5 = CStringUtility::convertCStringToStdString(cstr5);
			pBuildTXDDialogData->m_uiTextureCountPerTXD = str5 == "" ? 0 : CStringUtility::toNumber(str5);
		...
		case 114:
			pBuildTXDDialogData->m_strDFFsFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_8"), getKGM()->getLastUsedDirectory("BUILDTXD__DFF"));
			if (pBuildTXDDialogData->m_strDFFsFolderPath == "")
			{
				break;
			}
			pBuildTXDDialogData->m_strDFFsFolderPath = CPathUtility::addSlashToEnd(pBuildTXDDialogData->m_strDFFsFolderPath);
			getKGM()->setLastUsedDirectory("BUILDTXD__DFF", pBuildTXDDialogData->m_strDFFsFolderPath);
			((CEdit*)pDialog->GetDlgItem(106))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pBuildTXDDialogData->m_strDFFsFolderPath).c_str());
			((CEdit*)pDialog->GetDlgItem(113))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pBuildTXDDialogData->m_strDFFsFolderPath).c_str());
			break;
		case 111: // Change button for IDE file path
			vecFilePaths = CGUIUtility::openFileDialog(getKGM()->getLastUsedDirectory("BUILDTXD__IDE"), "IDE", false);
			if (vecFilePaths.size() == 0)
			{
				break;
			}
			...
			((CEdit*)pDialog->GetDlgItem(110))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pBuildTXDDialogData->m_strIDEFilePath).c_str());
			break;
		case 253: // Change button for textures folder path
			pBuildTXDDialogData->m_strTexturesFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_9"), getKGM()->getLastUsedDirectory("BUILDTXD__TEXTURES"));
			if (pBuildTXDDialogData->m_strTexturesFolderPath == "")
			{
				break;
			}
			getKGM()->setLastUsedDirectory("BUILDTXD__TEXTURES", pBuildTXDDialogData->m_strTexturesFolderPath);
			((CEdit*)pDialog->GetDlgItem(251))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pBuildTXDDialogData->m_strTexturesFolderPath).c_str());
			break;
		case 256: // Change button for destination folder path
			pBuildTXDDialogData->m_strDestinationFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_10"), getKGM()->getLastUsedDirectory("BUILDTXD__DESTINATION"));
			if (pBuildTXDDialogData->m_strDestinationFolderPath == "")
			{
				break;
			}
			getKGM()->setLastUsedDirectory("BUILDTXD__DESTINATION", pBuildTXDDialogData->m_strDestinationFolderPath);
			((CEdit*)pDialog->GetDlgItem(255))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pBuildTXDDialogData->m_strDestinationFolderPath).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CBuildTXDDialogData*		CGUIManager::showBuildTXDDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "IMGVersionSettings");
...
CIMGVersionSettingsDialogData*		CGUIManager::showIMGVersionSettingsDialog(void)
{
	...
	)
{
	...
	case WM_COMMAND:
		...
		case 150: // organize
		{
			...
			string strString1 = CStringUtility::convertCStringToStdString(cstr1);
			pTXDOrganizerDialogData->m_uiEveryNDFFFiles = CStringUtility::toNumber(strString1);
			pTXDOrganizerDialogData->m_strTXDNamePrefix = CStringUtility::convertCStringToStdString(cstr2);
			pTXDOrganizerDialogData->m_strTextureImportFolderPath = CStringUtility::convertCStringToStdString(cstr3);
			pTXDOrganizerDialogData->m_strIDEUpdateFolderPath = CStringUtility::convertCStringToStdString(cstr4);
			pTXDOrganizerDialogData->m_strOutputFolderPath = CStringUtility::convertCStringToStdString(cstr5);

			if (strString1 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_24"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}

			if (pTXDOrganizerDialogData->m_strTXDNamePrefix == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_25"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}

			if (pTXDOrganizerDialogData->m_strTextureImportFolderPath == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_26"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
				break;
			}

			if (pTXDOrganizerDialogData->m_strOutputFolderPath == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_28"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
				break;
			}
			...
			if (((CButton*)pDialog->GetDlgItem(107))->GetCheck() == BST_CHECKED)
			{
				...
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_30"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
					break;
				}
			}
			...
			pTXDOrganizerDialogData->m_strTextureImportFolderPath = CPathUtility::addSlashToEnd(pTXDOrganizerDialogData->m_strTextureImportFolderPath);
			pTXDOrganizerDialogData->m_strIDEUpdateFolderPath = CPathUtility::addSlashToEnd(pTXDOrganizerDialogData->m_strIDEUpdateFolderPath);
			pTXDOrganizerDialogData->m_strOutputFolderPath = CPathUtility::addSlashToEnd(pTXDOrganizerDialogData->m_strOutputFolderPath);

			EndDialog(hwndDlg, 0);
			break;
		}
		case 106: // Change button for Texture Import Folder Path
			pTXDOrganizerDialogData->m_strTextureImportFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_18"), getKGM()->getLastUsedDirectory("TXDORGANIZER_TEXTUREIMPORT"));
			if (pTXDOrganizerDialogData->m_strTextureImportFolderPath == "")
			{
				break;
			}
			...
			((CEdit*)pDialog->GetDlgItem(105))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTXDOrganizerDialogData->m_strTextureImportFolderPath).c_str());
			break;
		case 109: // Change button for IDE Update Folder Path
			pTXDOrganizerDialogData->m_strIDEUpdateFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_19"), getKGM()->getLastUsedDirectory("TXDORGANIZER_IDEUPDATE"));
			if (pTXDOrganizerDialogData->m_strIDEUpdateFolderPath == "")
			{
				break;
			}
			...
			((CEdit*)pDialog->GetDlgItem(108))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTXDOrganizerDialogData->m_strIDEUpdateFolderPath).c_str());
			break;
		case 112: // Change button for Output Folder
			pTXDOrganizerDialogData->m_strOutputFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_20"), getKGM()->getLastUsedDirectory("TXDORGANIZER_OUTPUTFOLDER"));
			if (pTXDOrganizerDialogData->m_strOutputFolderPath == "")
			{
				break;
			}
			...
			((CEdit*)pDialog->GetDlgItem(111))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pTXDOrganizerDialogData->m_strOutputFolderPath).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CTXDOrganizerDialogData*			CGUIManager::showTXDOrganizerDialog(void)
{
	...
	)
{
	...
	case WM_COMMAND:
		...
		case 350: // move
		{
			...
			str1 = CStringUtility::convertCStringToStdString(cstr1);
			str2 = CStringUtility::convertCStringToStdString(cstr2);
			str3 = CStringUtility::convertCStringToStdString(cstr3);
			str4 = CStringUtility::convertCStringToStdString(cstr4);
			str5 = CStringUtility::convertCStringToStdString(cstr5);
			str6 = CStringUtility::convertCStringToStdString(cstr6);
			str7 = CStringUtility::convertCStringToStdString(cstr7);
			str8 = CStringUtility::convertCStringToStdString(cstr8);
			str9 = CStringUtility::convertCStringToStdString(cstr9);
			str10 = CStringUtility::convertCStringToStdString(cstr10);
			str11 = CStringUtility::convertCStringToStdString(cstr11);

			pDATPathsMoverDialogData->m_strInputFolderPath = CStringUtility::convertCStringToStdString(cstr10);
			pDATPathsMoverDialogData->m_strOutputFolderPath = CStringUtility::convertCStringToStdString(cstr11);

			if (str1 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_31", "min", "x"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (str2 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_31", "min", "y"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (str3 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_31", "max", "x"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (str4 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_31", "max", "y"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}

			if (str5 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_35", "width"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (str6 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_35", "height"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}

			if (str7 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_57", "x"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (str8 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_57", "y"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (str9 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_57", "z"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}

			if (pDATPathsMoverDialogData->m_strInputFolderPath == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_58"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
				break;
			}

			if (pDATPathsMoverDialogData->m_strOutputFolderPath == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_59"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_PathNotSet").c_str());
				break;
			}

			pDATPathsMoverDialogData->m_vecOutputMapRangeMin.m_x = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr1));
			pDATPathsMoverDialogData->m_vecOutputMapRangeMin.m_y = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr2));
			pDATPathsMoverDialogData->m_vecOutputMapRangeMax.m_x = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr3));
			pDATPathsMoverDialogData->m_vecOutputMapRangeMax.m_y = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr4));

			pDATPathsMoverDialogData->m_vecOutputTileSize.m_x = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr5));
			pDATPathsMoverDialogData->m_vecOutputTileSize.m_y = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr6));

			pDATPathsMoverDialogData->m_vecPositionOffset.m_x = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr7));
			pDATPathsMoverDialogData->m_vecPositionOffset.m_y = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr8));
			pDATPathsMoverDialogData->m_vecPositionOffset.m_z = CStringUtility::toFloat32(CStringUtility::convertCStringToStdString(cstr9));

			pDATPathsMoverDialogData->m_strInputFolderPath = CPathUtility::addSlashToEnd(pDATPathsMoverDialogData->m_strInputFolderPath);
			pDATPathsMoverDialogData->m_strOutputFolderPath = CPathUtility::addSlashToEnd(pDATPathsMoverDialogData->m_strOutputFolderPath);

			EndDialog(hwndDlg, 0);
			break;
		}
		case 252: // Change button for Input Folder
			pDATPathsMoverDialogData->m_strInputFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_21"), getKGM()->getLastUsedDirectory("DATPATHSMOVER_DATINPUT"));
			if (pDATPathsMoverDialogData->m_strInputFolderPath == "")
			{
				break;
			}
			...
			((CEdit*)pDialog->GetDlgItem(251))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDATPathsMoverDialogData->m_strInputFolderPath).c_str());
			break;
		case 302: // Change button for Output Folder
			pDATPathsMoverDialogData->m_strOutputFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_22"), getKGM()->getLastUsedDirectory("DATPATHSMOVER_DATOUTPUT"));
			if (pDATPathsMoverDialogData->m_strOutputFolderPath == "")
			{
				break;
			}
			...
			((CEdit*)pDialog->GetDlgItem(301))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDATPathsMoverDialogData->m_strOutputFolderPath).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CDATPathsMoverDialogData*			CGUIManager::showDATPathsMoverDialogData(void)
{
	...
	)
{
	...
	case WM_COMMAND:
		...
		case 350: // go
		{
			...
			if (str1 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_62"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (str2 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_3"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}
			if (pMapMoverAndIDShifterDialogData->m_bUpdateIDEInFolder)
			{
				if (str3 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_64"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
			}
			if (pMapMoverAndIDShifterDialogData->m_bUpdateIPLInFolder)
			{
				if (str4 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_65"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
			}
			if (str5 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_28"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}

			pMapMoverAndIDShifterDialogData->m_strGameFolderPath = CPathUtility::addSlashToEnd(pMapMoverAndIDShifterDialogData->m_strGameFolderPath);
			pMapMoverAndIDShifterDialogData->m_strUpdateIDEInFolderPath = CPathUtility::addSlashToEnd(pMapMoverAndIDShifterDialogData->m_strUpdateIDEInFolderPath);
			pMapMoverAndIDShifterDialogData->m_strUpdateIPLInFolderPath = CPathUtility::addSlashToEnd(pMapMoverAndIDShifterDialogData->m_strUpdateIPLInFolderPath);
			...
			pMapMoverAndIDShifterDialogData->m_uiIDStart = CStringUtility::toNumber(str6);
			pMapMoverAndIDShifterDialogData->m_vecPositionOffset.m_x = CStringUtility::toFloat32(str7);
			pMapMoverAndIDShifterDialogData->m_vecPositionOffset.m_y = CStringUtility::toFloat32(str8);
			pMapMoverAndIDShifterDialogData->m_vecPositionOffset.m_z = CStringUtility::toFloat32(str9);

			if (pMapMoverAndIDShifterDialogData->m_bMovePosition)
			{
				if (str6 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_66"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
			}
			else if (pMapMoverAndIDShifterDialogData->m_bShiftIds)
			{
				if (str7 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_67", "X"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
				if (str8 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_67", "Y"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
				if (str9 == "")
				{
					CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopup_67", "Z"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
					break;
				}
			}
			else
			{
				CGUIUtility::showMessage("You must choose an action (ID Shifter or Map Mover)", "Action Choice Required");
				break;
			}

			for (string strFilename : CStringUtility::split(str10, "\n"))
			{
				pMapMoverAndIDShifterDialogData->m_umapShifterIgnoreFilenames[strFilename] = true;
			}
			for (string strFilename : CStringUtility::split(str11, "\n"))
			{
		...
		{
			vector<string> vecFilePaths = CGUIUtility::openFileDialog(getKGM()->getLastUsedDirectory("MAPMOVERANDIDSHIFTER_DATINPUT"), "DAT", false);
			if (vecFilePaths.size() == 0)
			{
				break;
			}
			pMapMoverAndIDShifterDialogData->m_strDATFilePath = vecFilePaths[0];
			getKGM()->setLastUsedDirectory("MAPMOVERANDIDSHIFTER_DATINPUT", CPathUtility::getDirectory(pMapMoverAndIDShifterDialogData->m_strDATFilePath));
			((CEdit*)pDialog->GetDlgItem(101))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pMapMoverAndIDShifterDialogData->m_strDATFilePath).c_str());
			break;
		}
		case 105: // Change button for Game Folder
			pMapMoverAndIDShifterDialogData->m_strGameFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_3"), getKGM()->getLastUsedDirectory("MAPMOVERANDIDSHIFTER_GAMEFOLDER"));
			...
			((CEdit*)pDialog->GetDlgItem(104))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pMapMoverAndIDShifterDialogData->m_strGameFolderPath).c_str());
			break;
		case 108: // Change button for IDE Folder
			pMapMoverAndIDShifterDialogData->m_strUpdateIDEInFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_5"), getKGM()->getLastUsedDirectory("MAPMOVERANDIDSHIFTER_IDEFOLDER"));
			...
			((CEdit*)pDialog->GetDlgItem(107))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pMapMoverAndIDShifterDialogData->m_strUpdateIDEInFolderPath).c_str());
			break;
		case 111: // Change button for IPL Folder
			pMapMoverAndIDShifterDialogData->m_strUpdateIPLInFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_6"), getKGM()->getLastUsedDirectory("MAPMOVERANDIDSHIFTER_IPLFOLDER"));
			...
			((CEdit*)pDialog->GetDlgItem(110))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pMapMoverAndIDShifterDialogData->m_strUpdateIPLInFolderPath).c_str());
			break;
		case 114: // Change button for Output Folder
			pMapMoverAndIDShifterDialogData->m_strOutputFolderPath = CGUIUtility::chooseFolderDialog(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_28"), getKGM()->getLastUsedDirectory("MAPMOVERANDIDSHIFTER_OUTPUTFOLDER"));
			if (pMapMoverAndIDShifterDialogData->m_strOutputFolderPath == "")
			{
				break;
			}
			pMapMoverAndIDShifterDialogData->m_strOutputFolderPath = CPathUtility::addSlashToEnd(pMapMoverAndIDShifterDialogData->m_strOutputFolderPath);
			getKGM()->setLastUsedDirectory("MAPMOVERANDIDSHIFTER_OUTPUTFOLDER", CPathUtility::getDirectory(pMapMoverAndIDShifterDialogData->m_strOutputFolderPath));
			((CEdit*)pDialog->GetDlgItem(113))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pMapMoverAndIDShifterDialogData->m_strOutputFolderPath).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CMapMoverAndIDShifterDialogData*		CGUIManager::showMapMoverAndIDShifterDialog(void)
{
	...
	)
{
	...
	case WM_COMMAND:
		...
		case 150: // fetch
		{
			...
			if (str3 == "")
			{
				CGUIUtility::showMessage(CLocalizationManager::getInstance()->getTranslatedText("TextPopup_63"), CLocalizationManager::getInstance()->getTranslatedFormattedText("TextPopupTitle_InputRequired").c_str());
				break;
			}

			pDATModelListDialogData->m_strGameFolderPath = CPathUtility::addSlashToEnd(pDATModelListDialogData->m_strGameFolderPath);
		...
		case 102: // Change button for DAT File
		{
			vector<string> vecFilePaths = CGUIUtility::openFileDialog(getKGM()->getLastUsedDirectory("DATMODELLIST_DATINPUT"), "DAT", false);
			if (vecFilePaths.size() == 0)
			{
				break;
			}
			pDATModelListDialogData->m_strDATFilePath = vecFilePaths[0];
			getKGM()->setLastUsedDirectory("DATMODELLIST_DATINPUT", CPathUtility::getDirectory(pDATModelListDialogData->m_strDATFilePath));
			((CEdit*)pDialog->GetDlgItem(101))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDATModelListDialogData->m_strDATFilePath).c_str());
			break;
		}
		case 105: // Change button for Game Folder
			pDATModelListDialogData->m_strGameFolderPath = CGUIUtility::chooseFolderDialog(hwndDlg, CLocalizationManager::getInstance()->getTranslatedText("ChooseFolderPopup_3"), getKGM()->getLastUsedDirectory("DATMODELLIST_GAMEFOLDER"));
			...
			((CEdit*)pDialog->GetDlgItem(104))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDATModelListDialogData->m_strGameFolderPath).c_str());
			break;
		case 108: // Change button for Output File
			pDATModelListDialogData->m_strOutputFilePath = CGUIUtility::saveFileDialog(getKGM()->getLastUsedDirectory("DATMODELLIST_TXTOUTPUT"), "TXT", "Model List.txt");
			if (pDATModelListDialogData->m_strOutputFilePath == "")
			{
				break;
			}
			getKGM()->setLastUsedDirectory("DATMODELLIST_TXTOUTPUT", CPathUtility::getDirectory(pDATModelListDialogData->m_strOutputFilePath));
			((CEdit*)pDialog->GetDlgItem(107))->SetWindowTextW(CStringUtility::convertStdStringToStdWString(pDATModelListDialogData->m_strOutputFilePath).c_str());
			break;
		}
		break;
	case WM_CLOSE:
...
CDATModelListDialogData*				CGUIManager::showDATModelListDialog(void)
{
	...
	)
{
	...
	case WM_INITDIALOG:
		...
		//getKGM()->getGUIManager()->updateGUIControlsTextToLanguage(pDialog, "OverwriteEntry");
...
COverwriteEntryDialogData*				CGUIManager::showOverwriteEntryDialog(void)
{
