...
class CIPLEntry_OCCL : public CIPLEntry_Data
{
public:
	CIPLEntry_OCCL(CIPLFormat *pIPLFormat) :
	...
	void						setMidPosition(CVector2D& vecMidPosition) { m_vecMidPosition = vecMidPosition; }
	CVector2D&					getMidPosition(void) { return m_vecMidPosition; }
	...
	void						setWidth(CVector2D& vecWidth) { m_vecWidth = vecWidth; }
	CVector2D&					getWidth(void) { return m_vecWidth; }
...
private:
	// GTA VC only
	CVector2D					m_vecMidPosition;
	float32						m_fBottomHeightZ;
	CVector2D					m_vecWidth;
	float32						m_fHeight;
	float32						m_fRotation;
