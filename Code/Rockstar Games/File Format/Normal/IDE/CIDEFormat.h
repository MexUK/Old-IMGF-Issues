...
class CIDEFormat : public CSectionLinesFormat<CIDEFormat, CIDEEntry, eIDESection, CIDEEntry_Other, CIDEEntry_Section, CIDEEntry_Data>
{
public:
	CIDEFormat(void);

	CIDEEntry_Data*								createDataEntry(eIDESection eIDESectionValue, uint32 uiSectionSpecificType = 0);
	
	template<class EntryClass2>
	std::vector<EntryClass2*>					getEntriesBySection(eIDESection eIDESectionValue, e2DFXType e2DFXTypeValue = _2DFX_UNKNOWN);
...
private:
	...
	CIDEEntry_Data*								unserializeDataEntry(eIDESection eIDESectionValue);

	std::pair<e2DFXType, uint32>				detect2DFXTypeAndGame(void);
	eIDEPathType								detectPATHType(void);
};

template<class EntryClass2>
std::vector<EntryClass2*>						CIDEFormat::getEntriesBySection(eIDESection eIDESectionValue, e2DFXType e2DFXTypeValue)
{
	if (eIDESectionValue != IDE_SECTION_2DFX || e2DFXTypeValue == _2DFX_UNKNOWN)
	{
